...
async function importTechnologiesFromWiki() {
    return loadPageContent(TECHNOLOGY_OVERVIEW_PAGE)
        .then(parseTechnologyOverview);
}
async function parseTechnologyOverview(body) {
    // console.log(body);
...
async function importTechnologyTree(treeTable, treeIndex) {
    ...
    const levels = await Promise.all(_.map(levelElements, (levelTable, level) => importTechnologyTreeLevel(levelTable, level, title)));
    return {
        name: title,
        levels: levels,
    };
}
async function importTechnologyTreeLevel(table, level, technologyTree) {
    const technologyElements = xpath.select(`./tr[position()>1]/th/a/@href`, table);
    const technologies = await Promise.all(technologyElements.map(technologyElement => importTechnology(technologyElement.value, level, technologyTree)));
    return {
        technologyTree,
        level,
